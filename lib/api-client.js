...
class ApiClient{
  ...
  async sendMessageWithHistory(model, messageHistory) {
     return await this.retryHandler.executeWithRetry(async () => {
      ...
      const aiResponse = response.data.choices[0].message.content;
      return aiResponse;
    }, `Chat API request to ${model}`);
  }

  /**
   * Send a message with conversation history and project context
   * @param {string} model - The model to use
   * @param {Array} messageHistory - Array of conversation messages
   * @param {string} context - Project context to include
   * @returns {Promise<string>} The AI's response
   */
  async sendMessageWithContext(model, messageHistory, context = '') {
    return await this.retryHandler.executeWithRetry(async () => {
      let messages = [...messageHistory];
      
      // If we have context and messages, prepend context to the last user message
      if (context && messages.length > 0) {
        const lastMessage = messages[messages.length - 1];
        if (lastMessage.role === 'user') {
          // Create a new message with context prepended
      ...
      const aiResponse = response.data.choices[0].message.content;
      return aiResponse;
    }, `Context-aware API request to ${model}`);
  }
