...
class ChatSession {
  ...
  async processUserMessage(userMessage) {
    try {
      ...
      console.error(chalk.yellow('💡 You can try:'));
      console.error(chalk.yellow('   • Rephrasing your question'));
      console.error(chalk.yellow('   • Checking your internet connection'));
      console.error(chalk.yellow('   • Trying again in a moment'));
      
      // Remove the failed user message from history so conversation continues properly
  ...
  start() {
    ...
    const fileCount = this.fileManager.getProjectFiles().length;
    if (fileCount > 0) {
      console.log(chalk.gray(`📁 Project: ${fileCount} files scanned`));
      console.log(chalk.gray('💡 Mention filenames in your questions, or type "files" to see all files'));
    } else {
      console.log(chalk.yellow('📁 No project files found in current directory'));
    }
